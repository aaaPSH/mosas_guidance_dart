...
void test_axis_aligned_velocity_decouples_pitch_and_yaw_overload() {
    ...
    const PngGuidanceConfig config{2.0, 4.0, kGravity};
    ...
    expect_near(output.navigation_acceleration.y, kGravity + 20.0);
    expect_near(output.navigation_acceleration.z, -80.0);
    expect_near(output.body_overload.y, 1.0 + 20.0 / kGravity);
    expect_near(output.body_overload.z, -80.0 / kGravity);
    expect_near(output.command_overload,
                std::hypot(1.0 + 20.0 / kGravity, -80.0 / kGravity));
    expect_near(output.command_phase,
                std::atan2(-80.0 / kGravity, 1.0 + 20.0 / kGravity));
}
...
void test_oblique_velocity_uses_speed_and_ballistic_pitch_compensation() {
    ...
    const EulerAngles attitude{0.0, 0.0, 0.0};
    const PngGuidanceConfig config{3.0, 3.0, kGravity};
